...
public class New implements LogConsts {
    ...
    public static synchronized Reusable get (Class cls) {
        ...
        try {
            if (D) Log.debug("create object "+cls.getName());
            return (Reusable)cls.newInstance();
        } catch (Exception e) {
            e.printStackTrace();
            throw new RemoteException("no default constructor in "+cls);
        }
    }
    ...
    public static Float f32 (float f) {
        return new Float(f);
    }
